"""
Train SB3 DQN on CartPole-v1 across multiple seeds and save an evaluation-curve plot.

This is the "calibrated baseline" entry point for Block 03.
...
What this script does:
  1) For each seed, train an SB3 DQN model on CartPole-v1 for ``--total-steps``
     env steps.
...
CartPole-v1. Pure library defaults are tuned for long Atari-scale runs and
learn CartPole only slowly, so the Zoo values are used to keep this baseline
honest at a small step budget.

Plot saved (in --plot-dir):
  sb3_dqn_cartpole_eval_returns_mean_std.png   -- SB3 DQN eval return
...
Quickstart:
  ...
  python examples/03_dqn/train_sb3_cartpole.py --total-steps 80000 --n-seeds 5
"""
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install -e '.[sb3]'"
    ) from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Stable-Baselines3 DQN baseline on CartPole-v1.",
...
def _train_sb3_run(
    *,
    ...
    # RL Baselines3 Zoo hyperparameters for CartPole-v1. Library defaults are
    # tuned for much longer runs, so without these the baseline barely learns
    # inside the small default step budget.
    model = DQN(
        policy="MlpPolicy",                       # network type SB3 builds; MlpPolicy = fully connected net for vector obs
        env=train_env,                            # env used for training
        learning_rate=2.3e-3,                     # step size of the Adam optimiser
        batch_size=64,                            # transitions sampled from the replay buffer per gradient step
        buffer_size=100_000,                      # max transitions kept in the replay buffer
        learning_starts=1_000,                    # env steps of pure exploration before training begins (fills the buffer first)
        gamma=0.99,                               # discount factor for future rewards
        target_update_interval=10,                # gradient steps between hard copies of the online net into the target net
        train_freq=256,                           # env steps collected between training phases
        gradient_steps=128,                       # gradient updates done per training phase
        exploration_fraction=0.16,                # fraction of total steps over which epsilon is annealed down
        exploration_final_eps=0.04,               # epsilon value held after annealing finishes
